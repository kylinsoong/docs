...
* IPv4 地址长度为 32 为二进制数，由 4 组十进制数组成，4 组十进制数之间通过圆点点连接
...
*IPv4 地址被分为一下几个类型（为了更好的管理互联网网络）*

|===
|类型 |描述 |范围 |Max Hosts

|A
|第一个 10 进制数用来做网络 ID，后面三个 10 进制数用来做主机 ID
|0 - 126
|16 M

|B
|前两个 10 进制数用来做网络 ID，后面二个 10 进制数用来做主机 ID
|128 - 191
|64000

|C
|前三个十进制数用来做网络 ID，后面一个十进制数用来做主机 ID
|192 - 223
|254
...
NOTE: 如果第一个二进制位为 0，则属于 A 类地址；如果第一个二进制位为 1，则属于 B 类地址；如果开始的二进制位为 110，则属于 C 类地址。
